...
class State():
    ...
    def __init__(self, seq=[658], t0=None, tof=[], vrel=None, dv=[], dv_damon = [], mass=2000, m_star = 0, sum = 0, next_move=MOVE_TYPE.T0):
    ...
    def move(self, move):
	    ...
	    self.sum = T_SCALE[self.seq[-1]][self.tof[-1]] - T_SCALE[658][self.t0]
	    if len(self.tof) > 1:
	        t1 = self.tof[-2]
	    else:
		t1 = self.t0
